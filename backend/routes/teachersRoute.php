...
$id = end($uriParts);
if ($id === 'teachers' || $id === 'api') {
    $id = null;
}

if ($method === 'GET') {
    if ($id === 'me') {
        echo json_encode($teacherController->showMe($userId));
    } elseif ($id) {
        echo json_encode($teacherController->show($id, $userId, $userRole));
    } else {
        echo json_encode($teacherController->index($_GET, $userRole));
    }
} elseif ($method === 'POST') {
    $body = json_decode(file_get_contents("php://input"), true);
    echo json_encode($teacherController->create($body, $userRole));
} elseif ($method === 'PUT') {
    $body = json_decode(file_get_contents("php://input"), true);
    if ($id && $id !== 'me') {
        echo json_encode($teacherController->update($id, $body, $userId, $userRole));
    } else {
        http_response_code(400);
        echo json_encode(["success" => false, "message" => "Teacher ID required for update"]);
    }
} elseif ($method === 'DELETE') {
    if ($id && $id !== 'me') {
        echo json_encode($teacherController->destroy($id, $userRole));
    } else {
        http_response_code(400);
        echo json_encode(["success" => false, "message" => "Valid Teacher ID required for deletion"]);
    }
} else {
