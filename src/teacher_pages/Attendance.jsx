...
export default function Attendance() {
  ...
  const [attendance, setAttendance] = useState(() => {
    // Try to load saved attendance for today
    const dateStr = format(new Date(), "yyyy-MM-dd");
    const savedKey = `attendance_10-A_${dateStr}`;
    ...
    // Default attendance
    return studentsAttendance.reduce(
      (acc, student) => ({
        ...acc,
  ...
  const loadAttendance = () => {
    ...
    const savedKey = `attendance_${selectedClass}_${dateStr}`;
    const saved = localStorage.getItem(savedKey);
    if (saved) {
      const data = JSON.parse(saved);
      const loadedAttendance = data.attendance.reduce((acc, record) => {
        acc[record.studentId] = record.status;
        return acc;
      }, {});
  ...
  // Auto-load attendance when date or class changes
  useEffect(() => {
    loadAttendance();
  ...
  const saveAttendance = () => {
    const dateStr = format(selectedDate, "yyyy-MM-dd");
    const attendanceKey = `attendance_${selectedClass}_${dateStr}`;
    ...
    const attendanceData = {
      ...
      attendance: Object.entries(attendance).map(([studentId, status]) => ({
        ...
        studentName: studentsAttendance.find(s => s.id === parseInt(studentId))?.name,
        status,
      })),
    };
    ...
    try {
      ...
      // Also maintain a log of all attendance records
      const allAttendanceLog = JSON.parse(localStorage.getItem("attendance_log") || "[]");
      const existingIndex = allAttendanceLog.findIndex(
        (record) => record.class === selectedClass && record.date === dateStr
      );
      
      if (existingIndex >= 0) {
        allAttendanceLog[existingIndex] = attendanceData;
      } else {
        allAttendanceLog.push(attendanceData);
      }
      
      localStorage.setItem("attendance_log", JSON.stringify(allAttendanceLog));
      ...
      toast({
        ...
        description: `Class ${selectedClass} on ${format(selectedDate, "PPP")} - ${present} Present, ${absent} Absent, ${late} Late. Saved to database.`,
      });
    } catch (error) {
      toast({
        title: "❌ Error Saving Attendance",
        description: "There was an error saving attendance. Please try again.",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    <DashboardLayout pageTitle="Attendance">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2 sm:gap-4 mb-4 sm:mb-6">
        ...
        <Button onClick={saveAttendance} className="bg-primary hover:bg-primary/90 h-7 sm:h-8 md:h-10 text-[10px] xs:text-xs sm:text-sm px-2 sm:px-3 md:px-4 w-full sm:w-auto">
          Save Attendance
        </Button>
      </div>
